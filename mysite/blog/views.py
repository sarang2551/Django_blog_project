...
def post_search(request):
    ...
    if 'query' in request.GET:
        ...
        if form.is_valid():
            ...
            search_vector = SearchVector('title',weight='A') + SearchVector('body',weight='B') # Body relevnancy has a higher weight than body relevancy
            search_query = SearchQuery(query)
            # Search Rank to list the posts based on relevancy
            results = Post.published.annotate(search = search_vector,rank = SearchRank(search_vector,search_query)).filter(search=search_query).order_by('-rank') # List of posts that satisfy the search field
